...
class Line(object):
    ...
    def if_parallel_lines(self,l2):
        if self.normal_vector.parallel_check(l2.normal_vector):
            return True
        else:
            return False


    def if_equal_lines(self,l2):
        x = self.basepoint[0] - l2.basepoint[0]
        y = self.basepoint[1] - l2.basepoint[1]

        if self.normal_vector.orthogonal_check(Vector([x,y])):
            return True
        else:
            return False
